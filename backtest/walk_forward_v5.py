...
V4 (swing确认 + vol自适应) + ML filter:
  - 入场时提取特征 → XGBoost 预测胜率 → 只进评分 ≥ 阈值的交易
"""
import numpy as np
import pandas as pd
...
class TradeResult:
    ...
    day_of_week: int = 0
    vol_mult: float = 1.0
    ml_score: float = -1.0  # [V5] ML 评分 (-1 = 未评估)
...
class ActiveShape:
    ...
    vol_mult: float = 1.0
    d_zone_extreme: float = 0.0
    fib_score: float = 0.0  # [V5] 用于ML特征
    price_symmetry_score: float = 0.0
...
class WalkForwardEngineV5:
    ...
    def __init__(
        ...
        confirm_bars: int = 3,
        confirm_mode: str = "swing",
        vol_adaptive: bool = False,
        btc_atr_pct_ref: float = 0.003,
        # ── V5 ML filter ──
        ml_model_path: str = "",
        ...
        if ml_model_path and os.path.exists(ml_model_path):
            ...
            self._ml_model.load_model(ml_model_path)
        elif ml_model_path:
            print(f"[V5] WARNING: ML模型不存在 {ml_model_path}", flush=True)

    @property
    def use_ml_filter(self) -> bool:
        return self._ml_model is not None and self.ml_threshold > 0
    ...
    def _compute_adx(high, low, close, period=14):
        ...
        for i in range(period*2, n): adx[i] = adx[i-1]*(1-a) + dx[i]*a
        return adx

    def _get_vol_mult(self, atr_val: float, price: float) -> float:
        if not self.vol_adaptive or price <= 0: return 1.0
        return max(1.0, (atr_val / price) / max(self.btc_atr_pct_ref, 1e-10))

    # ================================================================
    # [V5] ML 特征提取 + 预测
    # ================================================================
    def _predict_ml(self, shape, entry_bar, high, low, close, opens, volume,
                    atr, ema20, ema50, index_arr, n) -> float:
        """提取入场特征 → XGBoost 预测胜率"""
        ...
        # 形态几何特征
        ab_dist = shape.ab_distance_pct
        bc_ab = shape.bc_ab_ratio
        qual = shape.quality_score
        ab_bars = shape.ab_bars
        bc_bars = shape.bc_bars

        # D_zone 特征
        c_idx = shape.c_idx
        d_lower = shape.d_lower
        d_upper = shape.d_upper
        zone_bars = 0
        for j in range(c_idx + 1, entry_bar + 1):
            if j < n and low[j] <= d_upper and high[j] >= d_lower:
                zone_bars += 1
        ...
        dzone_vol_ratio = float(np.mean(dzone_vol) / max(vol_ma20, 1e-10))

        if entry_bar >= 5:
            dzone_mom = float((close[entry_bar] - close[entry_bar - 5]) / max(close[entry_bar - 5], 1e-10))
        else:
            dzone_mom = 0.0

        body = abs(close[entry_bar] - opens[entry_bar])
        total_range = high[entry_bar] - low[entry_bar]
        dzone_wick = float((total_range - body) / max(total_range, 1e-10))

        # 波动率
        atr_pct = float(atr[min(entry_bar, len(atr) - 1)] / entry_price)
        if entry_bar >= 20:
            atr_window = atr[entry_bar - 19:entry_bar + 1] / entry_price
            atr_p80 = float(np.percentile(atr_window, 80))
        else:
            atr_p80 = atr_pct

        # 趋势
        if entry_bar >= 14:
            ema_dir = 1 if ema20[entry_bar] > ema50[entry_bar] else -1
            price_vs_e20 = float(close[entry_bar] / max(ema20[entry_bar], 1e-10) - 1)
        else:
            ema_dir = 0; price_vs_e20 = 0.0

        # 成交量
        if entry_bar >= 20:
            vol_20 = np.mean(volume[entry_bar - 19:entry_bar + 1])
            vol_5 = np.mean(volume[max(0, entry_bar - 4):entry_bar + 1])
            vol_ratio = float(volume[entry_bar] / max(vol_20, 1e-10))
            vol_trend = float(vol_5 / max(vol_20, 1e-10))
        else:
            vol_ratio = 1.0; vol_trend = 1.0

        # 时间
        hour = index_arr[entry_bar].hour
        dow = index_arr[entry_bar].dayofweek

        # 组装特征向量（与训练时顺序一致）
        feat_vec = np.array([[
            ab_dist, bc_ab, qual, ab_bars, bc_bars,
            zone_bars, dzone_vol_ratio, dzone_mom, dzone_wick,
            atr_pct, atr_p80, ema_dir, price_vs_e20,
            vol_ratio, vol_trend, hour, dow, shape.vol_mult,
        ]], dtype=np.float32)

        proba = self._ml_model.predict_proba(feat_vec)[0]
        return float(proba[1])  # 返回 label=1 (胜) 的概率
    ...
    def run(self, df: pd.DataFrame, symbol: str, timeframe: str) -> pd.DataFrame:
        ...
        for i in range(start_bar, n):
            ...
            # ---- Step 3: 入场（含确认 + ML过滤） ----
            for shape in active_shapes:
                if shape.trade_entered: continue
                if shape.first_in_zone_bar is None: continue

                if self.use_confirmation:
                    confirmed, entry_bar = self._check_confirmation(
                        shape, i, close, opens, low, high)
                    if not confirmed:
                        if entry_bar == -1: shape.trade_entered = True
                        continue
                else:
                    entry_bar = i
                ...
                if self.use_ml_filter:
                    ml_score = self._predict_ml(
                        shape, entry_bar, high, low, close, opens, volume,
                        atr, ema20, ema50, df.index, n)
                ...
                if trade is not None:
                    trade.ml_score = ml_score  # [V5] 存到trade上(临时)
            ...
            for trade in active_trades:
                ...
                if result is not None:
                    ...
                    result.ml_score = getattr(trade, 'ml_score', -1.0)  # [V5]
        ...
        for trade in active_trades:
            ...
            result.vol_mult = trade.shape.vol_mult
            result.ml_score = getattr(trade, 'ml_score', -1.0)
            self._enrich_features(result, final_bar, high, low, close, opens,
                                  volume, atr, ema20, ema50, df, df.index)
            completed_trades.append(result)

        if ml_filtered_count > 0:
            print(f"  [V5] ML filtered: {ml_filtered_count} trades removed", flush=True)
    ...
    def _check_confirmation(self, shape, i, close, opens, low, high):
        ...
        if bars_in_zone > self.confirm_bars: return False, -1
        mode = self.confirm_mode; is_bull = shape.direction == "bullish"
        candle_ok = close[i] > opens[i] if is_bull else close[i] < opens[i]
        swing_ok = False
        if bars_in_zone >= 1:
            swing_ok = low[i] > shape.d_zone_extreme if is_bull else high[i] < shape.d_zone_extreme
        if mode == "candle": confirmed = candle_ok
        elif mode == "swing": confirmed = swing_ok
        else: confirmed = candle_ok and swing_ok
        if confirmed: return True, i
        return False, -2
    ...
    def _detect_all(self, current_bar, swings, atr, close, symbol, tf, adx_i):
        ...
        if s is not None and self._validate_shape(s, adx_i): shapes.append(s)
        multi = self._try_multi_patterns(swings, current_bar, atr, close, symbol, tf)
        for ms in multi:
            if self._validate_shape(ms, adx_i): shapes.append(ms)
        return shapes
    ...
    def _try_detect_abcd(self, swings, current_bar, atr, close, symbol, timeframe):
        ...
        b_idx = min(b[0], len(atr) - 1); b_atr = atr[b_idx]
        ...
        d_lower = d_proj * (1 - d_tol); d_upper = d_proj * (1 + d_tol)
        self._shape_counter += 1
        return ActiveShape(
            shape_id=f"{symbol}_{timeframe}_{direction}_{self._shape_counter}",
            direction=direction, a_price=a[1], b_price=b[1], c_price=c[1],
            d_projected=d_proj, bc_ab_ratio=round(bc_ab, 4),
            quality_score=round(quality, 1), ab_bars=ab_bars, bc_bars=bc_bars,
            cd_bars_estimated=int(ab_bars * self.cd_ab_ratio),
            a_idx=a[0], b_idx=b[0], c_idx=c[0], c_bar=current_bar,
            d_lower=d_lower, d_upper=d_upper, vol_mult=round(vol_mult, 3),
    ...
    def _try_multi_patterns(self, swings, current_bar, atr, close, symbol, timeframe):
        ...
        ab_len = abs(a[1] - b[1]); bc_len = abs(b[1] - c[1])
        if ab_len == 0: return []
        bc_ab = bc_len / ab_len; ab_bars = abs(b[0] - a[0]); bc_bars = abs(c[0] - b[0])
        ab_dist_pct = ab_len / b[1]
        d_proj = c[1] - ab_len * self.cd_ab_ratio if direction == "bullish" else c[1] + ab_len * self.cd_ab_ratio
        detected = detect_all_patterns(swings, bc_ab, self.cd_ab_ratio,
                                       ab_bars, bc_bars, ab_dist_pct, d_proj, direction)
        shapes = []
        b_idx = min(b[0], len(atr) - 1); b_atr = atr[b_idx]
        vol_mult = self._get_vol_mult(b_atr, b[1]); d_tol = self.d_zone_tolerance * vol_mult
        for hp in detected:
            ...
            shapes.append(ActiveShape(
                ...
                quality_score=hp.quality_score, ab_bars=ab_bars, bc_bars=bc_bars,
                ab_distance_pct=round(ab_dist_pct, 6), c_bar=current_bar,
                d_lower=hp.d_projected * (1 - d_tol), d_upper=hp.d_projected * (1 + d_tol),
    ...
    def _enter_trade(self, shape, current_bar, atr, close, df):
        ...
        return ActiveTrade(shape=shape, entry_bar=entry_bar, entry_price=entry_price,
    ...
    def _check_exit(self, trade, i, high, low, close, df):
        c_price = trade.shape.c_price; direction = trade.shape.direction
        ...
        if tp1_ok: trade.tp1_hit = True; trade.tp1_bar = i
        if sl_ok:
            exit_reason = "TP1+SL" if trade.tp1_hit else "SL"; exit_price = trade.sl_price
        elif be_ok and trade.tp1_hit:
            exit_reason = "TP1+BE"; exit_price = trade.be_price
        elif tp2_ok:
            exit_reason = "TP1+TP2" if trade.tp1_hit else "TP2"; exit_price = trade.tp2_price
    ...
    def _enrich_features(result, bar_i, high, low, close, opens, volume,
        ...
        for j in range(max(c_bar + 1, 0), entry_bar + 1):
            if j >= n: break
            if low[j] <= d_upper and high[j] >= d_lower: zone_bars += 1
